...
void Aia3::plotHough(vector< vector<Mat> > const& houghSpace){
    Mat hough = Mat::zeros(houghSpace.at(0).at(0).rows, houghSpace.at(0).at(0).cols, CV_32FC1);
    
    //cout << "scale:" << houghSpace.size() << endl;

    //loop trough scale
    for (int i = 0; i < houghSpace.size(); i++) {
        //cout << "angles:" << houghSpace.at(i).size() << endl;
        //trough angles
        for (int j = 0; j < houghSpace.at(i).size(); j++) {
            //loop trough vectors of one scale and angle
            for (int k = 0; k < houghSpace.at(i).at(j).rows; k++) {
                for (int l = 0; l < houghSpace.at(i).at(j).cols; l++) {
                    hough.at<float>(k, l) += houghSpace.at(i).at(j).at<float>(k, l);
                }
            }
        }
    }

    showImage(hough, "Hough", 0);
}
...
void Aia3::makeFFTObjectMask(vector<Mat> const& templ, double scale, double angle, Mat& fftMask){
    ...
    //Computes the spectrum of OiOb for correlation with the complex gradient image.
    //    ➔ Note: OiOb means component wise multiplication
    multiply(gradients[0],templ[0],gradients[0]);
    multiply(gradients[1],templ[0],gradients[1]);
    
    //showImage(gradients[0], "spectrum of correaltion with comp gradient x",0);
    //showImage(gradients[1], "spectrum of correaltion with comp gradient y",0);

    //Scales and rotates the object template
    Mat grad_x = rotateAndScale(gradients[0], angle, scale);
    Mat grad_y = rotateAndScale(gradients[1], angle, scale);
    
    //showImage(grad_x, "rotated, scaled x",0);
    //showImage(grad_y, "rotated, scaled y",0);
    //(incl. updating the phase of graients)
    Mat magnitude, phase;
    cartToPolar(grad_x, grad_y, magnitude, phase);
    Mat shift =  Mat::ones(phase.rows,phase.cols, CV_32FC1)*angle;
    add(phase, shift, phase);
    polarToCart(magnitude, phase, grad_x, grad_y);
    
    //showImage(grad_x, "updated gradients x",0);
    //showImage(grad_y, "updated gradients y",0);
    //followed by normalisation (eq.slide 34)
    double magSum = sum(magnitude)[0];
    divide(grad_x, magSum, grad_x);
    divide(grad_y, magSum, grad_y);

    //showImage(grad_x, "normalized grad x",0);
    //showImage(grad_y, "normalized grad y",0);
    //centring
    for(int i = 0; i < grad_x.rows; i++){
        for(int j = 0; j < grad_x.cols; j++){
            fftMask.at<Vec2f>(i,j)[0] = grad_x.at<float>(i,j);
            fftMask.at<Vec2f>(i,j)[1] = grad_y.at<float>(i,j);
        }
    }
    circShift(fftMask, fftMask, -binEdge.cols/2, -binEdge.rows/2);
    //Mat planes[2];
    //cv::split(fftMask, planes);

    //showImage(planes[0], "circ shift",0);
    //and Fourier transformation
    dft(fftMask,fftMask,DFT_COMPLEX_OUTPUT);
    //showImage(fftMask, "circ shift",0);
...
vector< vector<Mat> > Aia3::generalHough(Mat const& gradImage, vector<Mat> const& templ, double scaleSteps, double* scaleRange, double angleSteps, double* angleRange){
    ...
    cout << "max s" << scaleMax << "max angle: " << angleMax << endl;
    cout << "min s" << angleMin << "min angle: " << angleMin << endl;
    
    cout << "scaleSteps: " << scaleSteps << endl;
    cout << "angleSteps: " << angleSteps << endl;
    
    double scaleStep = (scaleMax - scaleMin)/ scaleSteps;
    double angleStep = (angleMax - angleMin)/ angleSteps;
    
    vector< vector<Mat> > hough;
    vector<Mat> orientation;
    Mat q;
    dft(gradImage, q);
    double scale = 0;
    cout << scaleStep << endl;
    ...
    for (int i = 0; i < scaleSteps; i++) {
        scale = scaleMin + i*scaleStep;
        double angle = 0;
        for (int j = 0; j < angleSteps; j++) {
            angle = angleMin + j*angleStep;
            cout << "angle: " << angle << endl;
            Mat fftMask = Mat::zeros(q.rows, q.cols, CV_32FC2);
       //     cout << "fftMask" << fftMask.size() << endl;
            makeFFTObjectMask(templ,scale,angle,fftMask);

            Mat convolution;
   //         cout << "query" << q.size() << endl;
     //       cout << "fftmask" << fftMask.size() << endl;
            mulSpectrums(q, fftMask, convolution, 0, true);
            dft(convolution, convolution, DFT_INVERSE);
            Mat transformed(convolution.rows,convolution.cols, CV_32FC1);
    
            for (int i = 0; i < transformed.rows; i++) {
                for (int j = 0; j < transformed.cols; j++) {
                    transformed.at<float>(i, j) = abs(convolution.at<Vec2f>(i, j)[0]);
                }
            }

            orientation.push_back(transformed);

        }
        hough.push_back(orientation);
    }
    return hough;
}
...
vector<Mat> Aia3::makeObjectTemplate(Mat const& templateImage, double sigma, double templateThresh){
    // calculate gradient of template as 2channel matrix
    Mat templateGradient = calcDirectionalGrad(templateImage, sigma);
    
    // split into mag and phase info
    Mat gradients[2];
    split(templateGradient, gradients);
    
    //Calculate the magnitude and phase of 2D vectors
    Mat magnitude = gradients[0];
    Mat phase = gradients[1];
    //return two mat structures, one with mag and second with phase of vectors
    cartToPolar(gradients[0], gradients[1], magnitude, phase);
    
    //get min and max magnitudes
    double maxG;
    minMaxLoc(magnitude, NULL, &maxG);
    
    //based on threshold calculate binary edge mask
    Mat binMask = Mat::zeros(magnitude.rows, magnitude.cols, CV_8UC2);
    int thresh = templateThresh*maxG;
    threshold(magnitude, binMask, thresh, 1, THRESH_BINARY);
    
    vector<Mat> templateVector;
    templateVector.push_back(binMask);
    templateVector.push_back(templateGradient);
...
void Aia3::run(string tmplImg, string testImg){
    ...
    double scaleSteps 		= 10;		// scale resolution in terms of number of scales to be investigated
...
void Aia3::process(Mat const& templateImage, Mat const& testImage, Mat const& params){
    ...
    //cout << "Complex part of template: "  << templ[1] << endl;
